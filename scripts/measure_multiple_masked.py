...
from stacktools.utils import get_segmentation, filter_segmentation_by_regions, get_wall_mask
...
def main(image_ds_uri, segmentation_dirpath, root_name, root_data_basepath):
    ...
    wall_mask = get_wall_mask(image_ds_uri, root_name)
    ...
    masked_segmentation = wall_mask * trimmed_segmentation

    file_ids = set(root_df.file_id)

    def measure_fid(fid):
        rids = list(root_df[root_df.file_id == fid].region_id)
        measure_df = measure_set_of_regions(masked_segmentation, masked_venus_stack, rids[:3])
    ...
    df_all.to_csv(f"{root_name}-multimeasure-masked-s.csv", float_format='%.3f', index=False)
